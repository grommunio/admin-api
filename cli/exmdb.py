...
def cliExmdbFolderPermissionsModify(args):
    ...
    with Service("exmdb") as exmdb:
        ...
        cli.table([(_FolderNode(folder).print(cli), _cliExmdbFolderPermissionPrint(cli, perm))
                   for folder, perm in zip(folders, perms)])
...
def cliExmdbFolderPermissionsShow(args):
    ...
    with Service("exmdb") as exmdb:
        ...
        cli.table([(member.mail, _cliExmdbFolderPermissionPrint(cli, member.rights))
                   for member in members if not member.special and (not args.username or member.mail in args.username)],
                  empty=cli.col("(no entries)", attrs=["dark"]))
